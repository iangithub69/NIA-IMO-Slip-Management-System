...
Public Class Form2_F
    ...
    Private Sub accountTypes()
        ...
        Dim sql As String = "SELECT account_id AS 'Account ID', account_no AS 'Account Number', account_name AS 'Account Name' FROM account_types WHERE account_id <> @excludedID"
    ...
    Private Sub Form2_F_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        accountTypes()
        ...
        Try
            ...
            If result = DialogResult.Yes Then
                ...
                Using connection As New MySqlConnection(connectionString)
                    connection.Open()
                    Dim sql As String = "UPDATE account_types SET account_no=@account_no, account_name=@account_name WHERE account_id = @account_id"
                    Dim command As New MySqlCommand(sql, connection)
                    command.Parameters.AddWithValue("@account_no", TextBox1.Text)
                    command.Parameters.AddWithValue("@account_name", TextBox2.Text)
                    command.Parameters.AddWithValue("@account_id", DataGridView1.SelectedRows(0).Cells(0).Value.ToString()) ' Assuming detail_id is in the first column
            ...
            If result = DialogResult.Yes Then
                ...
                Try
                    For Each selectedRow As DataGridViewRow In DataGridView1.SelectedRows
                        Dim accountID As String = selectedRow.Cells(0).Value.ToString() ' Assuming teller_id is in the first column

                        ' SQL query to delete the selected row based on the teller_id
                        Dim sql As String = "DELETE FROM account_types WHERE account_id = @accountID"
                        Dim cmd As New MySqlCommand(sql, cn)
    ...
    Private Sub InsertData()
        ...
        Try
            ...
            Dim sql As String = "INSERT INTO account_types (account_no, account_name) VALUES (@account_no, @account_name)"
            Dim cmd As New MySqlCommand(sql, cn)
            cmd.Parameters.AddWithValue("@account_no", TextBox3.Text)
            cmd.Parameters.AddWithValue("@account_name", TextBox4.Text)
